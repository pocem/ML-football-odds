...
UPDATE: greedy forward covariate search (models/tuning/Poisson_Bivariate_covariate_search.py),
informed by RF_feature_importance_engineered.py's Gini + permutation
importance results, found that adding Home_ShotOnTargetDifference_RollingTeam7
/ Away_ShotOnTargetDifference_RollingTeam7 (own value + opponent's value,
mirrored into both equations same as Elo) improves the intra-season
walk-forward pooled log loss from 0.9695 to 0.9678. No other single
candidate from a 13-feature pool cleared the improvement threshold, so this
is the only addition on top of the original 5 covariates.
"""
...
def walk_forward_vs_market(all_df):
    ...
    for i in range(WINDOW, len(seasons)):
        ...
        fold_rows.append({
            ...
            "avg_acc": avg_correct.mean(),
        })

        print(
            f"{test_season} (train {train_seasons[0]}..{train_seasons[-1]}): "
            f"model={model_ll.mean():.4f} bet365={b365_ll.mean():.4f} avg={avg_ll.mean():.4f} "
    ...
    print("\n=== Per-fold summary ===")
    print(fold_df[[
        "test_season", "n_test", "model_log_loss", "bet365_log_loss", "avg_log_loss",
        "model_acc", "bet365_acc", "avg_acc",
    ]].round(4))
