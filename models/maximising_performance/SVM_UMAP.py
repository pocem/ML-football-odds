"""
SVM.py, but with UMAP dimensionality reduction (91 features -> a handful of
components) before the RBF-kernel SVM, instead of hand-picking 8 covariates.

Going in, this was expected to be a longshot, for two reasons: (1) UMAP is
unsupervised, like PCA -- it preserves neighborhood/manifold structure in
FEATURE space, with no notion of what's predictive of match outcome, so
there's no guarantee its components carry the 1X2 signal; (2) stacking UMAP
under an RBF kernel means two layers of nonlinear transformation (UMAP's
manifold embedding, then the RBF kernel's own nonlinear similarity), which
is unusual and potentially unstable, especially since UMAP has to be
refit from scratch on a different ~1000-row training set every walk-forward
fold (StandardScaler and PCA are also refit per fold, but their fit is a
closed-form, stable computation; UMAP's is a stochastic, iterative manifold
optimization -- noisier by construction on this little data).
...
walkforward.py / FFNN.py / LogisticRegression.py / RF.py / XGBoost.py): each
test season is split into N_CHUNKS chronological pieces, and training grows
every chunk (prior WINDOW-VAL_SEASONS seasons + every earlier chunk of the
current season). The most recent of the WINDOW prior seasons is held out
fixed as a validation fold purely for the train/val overfitting diagnostic --
UMAP+SVC doesn't use it for any training decision, but keeping the same
WINDOW/VAL_SEASONS split as the other scripts keeps this comparable. UMAP is
refit from scratch every chunk on the chunk's own train_df, same leakage
...
N_COMPONENTS = 15  # matches the hand-picked covariate count in SVM.py, for a fair comparison
...
def main():
    ...
    print(f"Feature count (pre-UMAP): {all_df.drop(columns=drop_cols).shape[1]} -> UMAP components: {N_COMPONENTS}")
    ...
    for i in range(WINDOW, len(seasons)):
        ...
        for c_idx, test_chunk in enumerate(chunks):
            ...
            # Fit UMAP on TRAIN only, transform val/test through the same embedding
            reducer = UMAP(n_components=N_COMPONENTS, random_state=42)
            X_train = reducer.fit_transform(X_train_scaled)
            X_val = reducer.transform(X_val_scaled)
            X_test = reducer.transform(X_test_scaled)
