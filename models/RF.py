"""
Random Forest, 1X2 prediction, full feature set. Now on the "classic"
intra-season cumulative sliding walk-forward (same ablation as
RF_intraseason_walkforward.py / Poisson_Bivariate_intraseason_walkforward.py
/ FFNN.py / LogisticRegression.py): each test season is split into N_CHUNKS
chronological pieces, and training grows every chunk (prior WINDOW-VAL_SEASONS
seasons + every earlier chunk of the current season), instead of the old
season-level version that only ever trained on whole prior seasons and
tested the whole next season in one shot. VAL_SEASONS=1 (the most recent of
the WINDOW prior seasons) is kept fixed as a validation fold purely for the
train/val overfitting diagnostic -- RF doesn't need it for any training
decision, but keeping the same WINDOW/VAL_SEASONS split as the other scripts
keeps this comparable.
...
def main():
    ...
    for i in range(WINDOW, len(seasons)):
        ...
        for c_idx, test_chunk in enumerate(chunks):
            ...
            train_ll, _ = eval_probs(rf, X_train, y_train, classes_order)
            val_ll, _ = eval_probs(rf, X_val, y_val, classes_order)
